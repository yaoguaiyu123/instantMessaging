    ...
    for (int nIndex = m_currIndex; nIndex < m_IIVec.count(); nIndex++)
    {
        ...
        case Text:
        {
            ...
            for (int i = 0; i < strMsgList.size(); i++) {
                if (nLineWidth < fm.horizontalAdvance(strMsgList.at(i)))
                {
                    nLineWidth = fm.horizontalAdvance(strMsgList.at(i));
...
//            bubbleWidth = bubbleWidth < 30 ? 30 : bubbleWidth;
        ...
        if (Right == nOrientation) {
            ...
            painter->drawText(nWidth - 55 -  fm.horizontalAdvance(strTitle)- 10, nY + fm.height() / 2, strTitle);
            painter->restore();
