...
//        int screenY=desktop.availableGeometry().height()-40;
        ...
        int screenY = screenGeometry.height() - 40;
...
//        int screenY = qApp->desktop()->availableGeometry().height()-60;
        ...
        QRect screenGeometry = primaryScreen->availableGeometry();
        int screenX = screenGeometry.width();
        int screenY = screenGeometry.height() - 60;
        int wndX = frm->width();
        int wndY = frm->height();
...
//        return RegExp.exactMatch(IP);
        ...
        // 检查是否有完全匹配
        bool isExactMatch = match.hasMatch() && match.capturedStart() == 0 && match.capturedEnd() == IP.length();
        return isExactMatch;
    }
